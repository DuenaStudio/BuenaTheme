...
function optionsframework_admin_notice() {
	...
	if ( !is_multisite() && ( $pagenow == 'plugins.php' || $pagenow == 'themes.php' ) ) {
		...
		if ( ! get_user_meta($user_id, 'optionsframework_ignore_notice') ) {
			...
			printf( __('Your current theme does not have support for the Options Framework plugin.  <a href="%1$s" target="_blank">Learn More</a> | <a href="%2$s">Hide Notice</a>', 'bueno'), 'http://wptheming.com/options-framework-plugin', '?optionsframework_nag_ignore=0');
			echo "</p></div>";
		}
	}
}
...
function optionsframework_menu_settings() {
	...
	$menu = array(
		'page_title' => __( 'Theme Options', 'bueno'),
		'menu_title' => __('Theme Options', 'bueno'),
...
function optionsframework_load_scripts( $hook ) {
	...
	if ( !wp_script_is( 'wp-color-picker', 'registered' ) ) {
		...
		$colorpicker_l10n = array(
			'clear' => __( 'Clear', 'bueno' ),
			'defaultString' => __( 'Default', 'bueno' ),
			'pick' => __( 'Select Color', 'bueno' )
		);
